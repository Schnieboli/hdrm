...
test_that("AM representations agree for equal covariances", {
  ...
  expect_equal(
    result_am0$statistic,
    result_am1$statistic,
    tolerance = 1e-12
  )
...
test_that("B expressions are parsed without evaluating arbitrary R code", {
  ...
  expect_identical(
    hdrm:::evaluate_subsample_budget(
      B = 10,
      N = 16
    ),
    10L
...
test_that("equal-covariance subsampling budget is allocated correctly", {
  ...
  minimum_allocation <- hdrm:::allocate_C1_subsamples(
    group_sizes = c(6L, 7L, 8L),
    B = 1L
  )
  ...
  balanced_allocation <- hdrm:::allocate_C1_subsamples(
    group_sizes = c(6L, 6L, 6L),
    B = 8L
  )
  ...
  unbalanced_allocation <- hdrm:::allocate_C1_subsamples(
    group_sizes = c(6L, 7L, 8L),
    B = 20L
  )
...
test_that("matrix preprocessing is reflected in the returned object", {
  ...
  expect_equal(
    result$data,
    Matrixbirthrates[expected_order, , drop = FALSE]
  )
...
test_that("vector output reports subject counts rather than measurement counts", {
  ...
  subject_group_pairs <- unique(
    data.frame(
      subject = composite_subject,
      whole = droplevels(as.factor(EEG$group))
    )
  )

  expect_equal(
    result$groups$table,
    table(subject_group_pairs$whole)
  )
  expect_equal(
    nrow(result$data),
    nrow(subject_group_pairs)
  )
  expect_equal(result$removed.cases, 0L)
})
...
test_that("groups removed through missing data are dropped from the analysis", {
  ...
  removed_group <- levels(
    droplevels(as.factor(data_with_missing_group$group))
  )[[1L]]
  removed_index <- data_with_missing_group$group == removed_group
  removed_subjects <- length(
    unique(data_with_missing_group$subject[removed_index])
  )
  ...
  expect_equal(
    result$groups$a,
    nlevels(droplevels(data_with_missing_group$group[!removed_index]))
  )
...
test_that("an error is raised if fewer than two groups remain", {
  ...
  retained_group <- levels(
    droplevels(as.factor(data_with_one_group$group))
  )[[1L]]

  data_with_one_group$value[
    data_with_one_group$group != retained_group
  ] <- NA_real_
...
test_that("grouped C++ subsampling estimators have the intended kernels", {
  ...
  kernel_values <- A3_kernel_values(
    X_subsampling
  )
  ...
  standard_error <- sqrt(
    stats::var(kernel_values) / B_subsampling
  )
...
test_that("equal-covariance exact C++ estimators agree with R references", {
  ...
  C1_kernel_values <- function(X) {
    ...
    apply(
      ...
      function(index) {
        ...
        inner_product_12_34 <- sum(
          difference_12 * difference_34
        )
        inner_product_34_56 <- sum(
          difference_34 * difference_56
        )
        inner_product_56_12 <- sum(
          difference_56 * difference_12
        )
  ...
  # Independently calculated values for the fixed matrix
  expect_equal(
    hdrm:::A1_i_eq_cpp(X_fixed),
    73,
    tolerance = 1e-12
  )

  expect_equal(
    hdrm:::A2_i_eq_cpp(X_fixed),
    844,
    tolerance = 1e-12
  )

  expect_equal(
    hdrm:::C1_i_eq_cpp(X_fixed),
    12960,
    tolerance = 1e-12
  )
  ...
  for (current_matrix in matrices) {
    expect_equal(
      hdrm:::A1_i_eq_cpp(current_matrix),
      A1_eq_R_raw(current_matrix),
      tolerance = 1e-12
    )

    expect_equal(
      hdrm:::A2_i_eq_cpp(current_matrix),
      A2_eq_R_raw(current_matrix),
      tolerance = 1e-12
    )

    expect_equal(
      hdrm:::C1_i_eq_cpp(current_matrix),
...
test_that("equal-covariance C1 subsampling has the intended kernel", {
  ...
  kernel_values <- C1_kernel_values(
    X_fixed
  )
  expected_kernel_mean <- mean(
    kernel_values
  )

  expect_equal(
    expected_kernel_mean,
    18,
    tolerance = 1e-12
  )
  ...
  raw_estimate_1 <- hdrm:::C1star_i_eq_cpp(
    X_fixed,
    B_subsampling
  )
  ...
  raw_estimate_2 <- hdrm:::C1star_i_eq_cpp(
    X_fixed,
    B_subsampling
  )
  ...
  expect_lte(
    abs(estimate_per_draw - expected_kernel_mean),
    10 * standard_error + 1e-12
  )
  ...
  expect_lte(
    abs(
      raw_estimate_1 / (8 * B_subsampling) -
...
test_that("C5star_cpp_internal respects groups and matches its R kernel", {
  ...
  B_internal <- 20000L
  standard_error <- sqrt(
    stats::var(kernel_values) / B_internal
  )
...
test_that("grouped p-values use the stable upper chi-square tail", {
  ...
  expect_equal(
    heterogeneous_moderate$p.value,
  ...
  expect_equal(
    equal_covariance_moderate$p.value,
  ...
  expect_true(
    heterogeneous_moderate$p.value >= .Machine$double.eps
  )

  expect_true(
    equal_covariance_moderate$p.value >= .Machine$double.eps
  )
})
...
test_that("MSrootcompact remains an internal helper", {
  ...
  expect_false(
    "MSrootcompact" %in% getNamespaceExports("hdrm")
  )
...
test_that("predefined hypothesis labels are preserved in both covariance procedures", {
  ...
  for (hypothesis in hypotheses) {
    ...
    expect_identical(
      ...
      hypothesis,
      info = paste(
        "heterogeneous covariance procedure:",
        hypothesis
      )
    )
    ...
    expect_identical(
      ...
      hypothesis,
      info = paste(
        "equal covariance procedure:",
        hypothesis
      )
    )

    heterogeneous_output <- paste(
      capture.output(print(heterogeneous)),
      collapse = "\n"
    )

    homogeneous_output <- paste(
      capture.output(print(homogeneous)),
      collapse = "\n"
    )
    ...
    expect_match(
      ...
      fixed = TRUE,
      info = paste(
        "printed heterogeneous covariance result:",
        hypothesis
      )
    )
    ...
    expect_match(
      ...
      fixed = TRUE,
      info = paste(
        "printed equal covariance result:",
        hypothesis
      )
    )
  }
