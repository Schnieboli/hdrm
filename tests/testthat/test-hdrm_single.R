...
test_that("matrix preprocessing is reflected in the returned object", {
  ...
  expect_equal(
    result$data,
    data_with_na[complete_subjects, , drop = FALSE]
  )
  expect_equal(result$removed.cases, 1L)
...
test_that("within-subject measurement order is preserved", {
  ...
  subject_blocks <- split(
    seq_along(EEG$value),
    as.character(EEG$subject)
  )

  original_subject_order <- unique(
    as.character(EEG$subject)
  )
  reversed_subject_order <- rev(original_subject_order)

  permutation <- unlist(
    subject_blocks[reversed_subject_order],
    use.names = FALSE
  )

  result_original <- hdrm_single(
    EEG$value,
    hypothesis = "flat",
    subject = EEG$subject
  )

  result_permuted <- hdrm_single(
    EEG$value[permutation],
    hypothesis = "flat",
    subject = EEG$subject[permutation]
  )

  expected_subject_order <- match(
    reversed_subject_order,
    original_subject_order
  )

  expect_equal(
    result_permuted$data,
    result_original$data[
  ...
  expect_equal(
    result_permuted$statistic,
    result_original$statistic
  )
  expect_equal(
    result_permuted$p.value,
...
test_that("non-finite data and degenerate hypotheses are rejected", {
  ...
  expect_error(
    hdrm_single(
      Matrixbirthrates,
      hypothesis = hypothesis_with_inf
    ),
    paste0(
...
test_that("single-group C++ trace estimators agree with R references", {
  ...
  for (current_matrix in matrices) {
    expect_equal(
      hdrm:::B0_cpp(current_matrix),
      B0_R(current_matrix),
      tolerance = 1e-12
    )

    expect_equal(
      hdrm:::B2_cpp(current_matrix),
      B2_R(current_matrix),
      tolerance = 1e-12
    )
