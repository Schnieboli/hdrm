...
compute_grouped_df <- function(second_order,
  ...
  if (any(lengths(list(
    second_order, third_order, design_factor
  )) != 1L) ||
  ...
  if (second_order <= 0) {
    stop("The second-order trace estimate must be positive.", call. = FALSE)
  }
  ...
  if (design_factor <= 0) {
    stop("The design factor must be positive.", call. = FALSE)
  }
